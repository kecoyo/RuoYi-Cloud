    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" plain icon="Edit" :disabled="single" @click="handleEdit"
          v-hasPermi="['butler:iconType:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['butler:iconType:remove']">删除</el-button>
      </el-col>
...
import { listIconType, getIconType, addIconType, updateIconType, delIconType, changeIconTypeStatus } from "@/api/butler/iconType";
...
function getList() {
  loading.value = true;
  listIconType(proxy.addDateRange(queryParams.value, dateRange.value)).then(
    (response) => {
...
function handleEdit(row) {
  reset();
  const iconTypeId = row.id || ids.value;
  getIconType(iconTypeId).then(response => {
  ...
  proxy.$refs["formRef"].validate(valid => {
    if (valid) {
      if (form.value.id != undefined) {
        updateIconType(form.value).then(response => {
        ...
        addIconType(form.value).then(response => {
...
function handleStatusChange(row) {
  ...
  proxy.$modal.confirm('确认要"' + text + '""' + row.name + '"分类吗?').then(function () {
    return changeIconTypeStatus(row.id, row.status);
  }).then(() => {
